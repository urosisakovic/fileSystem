...
DWORD WINAPI nit2run(){
	...
	signal(sem21); // signalizira nit1
	{
		...
		while ((src = FS::open(filepath, 'r')) == 0) {
			wait(mutex);
			cout << threadName << ":Neuspesno otvoren fajl '" << filepath << "'" << endl;
			signal(mutex);
			Sleep(1); // Ceka 1 milisekundu
		}

		wait(mutex);
		cout << threadName << ": Otvoren fajl '" << filepath << "'" << endl;
		signal(mutex);
		...
		wait(mutex);
		cout << threadName << ": Prepisan fajl '" << filepath << "' u '" << filepath1 << "'" << endl;
		signal(mutex);

		delete dst;
		wait(mutex);
		cout << threadName << ": Zatvoren fajl '" << filepath1 << "'" << endl;
		signal(mutex);
	...
	{
		wait(mutex);
		cout << threadName << ": Broj fajlova na disku je " << FS::readRootDir() << endl;
		signal(mutex);
	}
...

	{
		...
		wait(mutex);
		cout<< threadName << ": Upisan '" << filepath << "' u fajl os domacina 'izlaz1.dat'"<<endl;
		signal(mutex);
...

	{
		...
		wait(mutex);
		cout << threadName << ": Otvoren fajl '" << copied_filepath << "' i dohvacena velicina" << endl;
		signal(mutex);

		delete copy;
		wait(mutex);
		cout << threadName << ": Zatvoren fajl '" << copied_filepath << "'" << endl;
		signal(mutex);
		...
		wait(mutex);
		cout << threadName << ": Prepisana druga polovina '" << filepath << "' u '" << filepath1 << "'" << endl;
		signal(mutex);
...

	{
		...
		fout.write(buff, f->getFileSize());
		wait(mutex);
		cout << threadName << ": Upisan '" << filepath << "' u fajl os domacina 'izlaz2.dat'" << endl;
		signal(mutex);
...

	{
		FS::unmount();
		wait(mutex);
		cout << threadName << ": Demontirana particija p1" << endl;
		signal(mutex);
	}
