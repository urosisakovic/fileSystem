...
DWORD WINAPI nit1run(){
	...
	wait(mutex); 
	cout<<threadName<<": Kreirana particija"<<endl; 
	signal(mutex);

	FS::mount(partition);
	wait(mutex);
	cout<<threadName<<": Montirana particija"<<endl;
	signal(mutex);

	FS::format();
	wait(mutex);
	cout<< threadName << ": Formatirana particija"<<endl;
	signal(mutex);
	...
	wait(sem21); //ceka nit2
	{
		char filepath[] = "/fajl1.dat";

		File *f=FS::open(filepath, 'w');
		wait(mutex);
		cout<< threadName << ": Kreiran fajl '"<< filepath <<"'"<<endl;
		signal(mutex);
